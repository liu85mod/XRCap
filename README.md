...
* **极致的 VR180 渲染**：支持 Cubemap（完美 180° 无死角覆盖）与 Foveated（注视点分环渲染）模式；支持 Fisheye、标准 ERP 及超级 ERP（折叠）投影输出。
* **8K 120FPS 10Bit录制**：支持高达 2x 的空间超采样（SSAA）抗锯齿，可直出 4:4:4 无损高保真 PNG，保留每一丝毛发细节。
* **3D 空间音频捕获**：内置独家定制的虚拟音频引擎（Virtual Audio Engine），在逐帧慢速渲染时也能完美抓取并合成随镜头移动的 3D 立体音效。
